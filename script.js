...
function renderArticles(articles) {
  ...
  if (!articles || articles.length === 0) {
    wrap.innerHTML = '<p>No articles found.</p>'; return;
  }
  articles.forEach(a => {
    const card = el('article','card');
    const img = document.createElement('img');
    img.src = a.urlToImage || 'https://via.placeholder.com/600x400?text=No+Image';
    const body = el('div','body');
    const h3 = el('h3'); h3.textContent = a.title;
    const meta = el('div','meta'); meta.textContent = `${a.source?.name || 'Unknown'} • ${timeAgo(a.publishedAt)}`;
...
function renderTrending(articles) {
  ...
  articles.slice(0,5).forEach(a => {
    ...
    ael.href = a.url; ael.textContent = a.title; ael.target = '_blank';
    li.append(ael); ul.append(li);
  });
}
...
async function fetchTopHeadlines({category=null, q=null}={}) {
  ...
  const params = new URLSearchParams({ pageSize: CONFIG.pageSize, country: CONFIG.country, apiKey: CONFIG.NEWSAPI_KEY });
...
document.getElementById('refresh').addEventListener('click', ()=>{
  ...
  load({category: active?.dataset.cat, q: document.getElementById('search').value});
});
